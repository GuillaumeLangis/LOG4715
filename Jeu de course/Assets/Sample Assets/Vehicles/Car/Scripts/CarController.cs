...
public class CarController : MonoBehaviour
{
    ...
    [SerializeField] int stylePointsPerAirFrame = 1;                                // Number of style points given each fixedUpdate in the air
    [SerializeField] int stylePointsPerNearCollision = 1;                           // Number of style points given each fixedUpdate near other car
    [SerializeField] float nearCollisionRadius = 3f;                                // Distance a laquelle les points de styles sont générés
    [SerializeField] LayerMask nearCollisionMask;

    [Header("Style Points")]
    public bool useNitro = false;                                                   // Is nitro in use?
    [Range(0f, 1f)] public float nitroLeft = 1f;                                    // Boost quantity left
    [SerializeField] float nitroTorqueFactor = 2f;                                  // Facteur dacceleration lorsque nitro est utilisee
    [SerializeField] float nitroMaxSpeedFactor = 2f;                                // Facteur de vitesse maximale lorse nitro est activee
    [SerializeField] float nitroRegenPerStylePoint = 0.1f;                          // Number of nitro regenerated for every style point gained
    [SerializeField] float nitroRegenPassive = 0.05f;                               // Number of nitro regenerated per second
    ...
    {
        _stylePoints += nb;
        nitroLeft += nitroRegenPerStylePoint * nb;
    }

    [Header("Car Damage")]
    [SerializeField] bool invulnerable = false;                                     // Determines whether the car can take damage or not
    [SerializeField] [Range(0f, 1f)] float _health = 1f;                             // Car health
    public float health
    {
        get
        {
    ...
    [SerializeField] float minHealthMaxSpeedFactor = 0.5f;                          // Factor of maximum speed when car health is at 0
    [SerializeField] float healthRegenPassive = 0.1f;                               // Passive health regeneration
    [SerializeField] float damagePerCarCollision = 0.25f;
    [SerializeField] float carCollisionRelativeVelocityThreshold = 2;               // Relative velocity magitude at which damage occurs
    ...
    public void Repair(float rep)
    {
        _health = Mathf.Clamp(health + rep, 0f, 1f);
    }

    [Header("Projectiles")]
    [SerializeField] Transform launchPt;
    [SerializeField] WaypointCircuit projCircuit;
    public Projectile projectilePrefab; // Projectile to be launched - can be null, in which case launching does nothing
    public void LaunchProjectile()
    {
        if (projectilePrefab)
        {
            if (projectilePrefab.projectileType == Projectile.ProjType.Green)
            {
                // Just launch at speed
                Projectile proj = Instantiate(projectilePrefab) as Projectile;
                proj.transform.position = launchPt.position;
                proj.transform.rotation = launchPt.rotation;

                proj.source = this;
                proj.body.velocity = transform.forward * proj.speed;

                // Clear the prefab so no projectile can be launched
                projectilePrefab = null;
            }
            else if (projectilePrefab.projectileType == Projectile.ProjType.Red)
            {
                // Find next car target...
            ...
            else if (projectilePrefab.projectileType == Projectile.ProjType.Blue)
            {
                // Find FIRST car in line, if it isn't you
                if (checkpointManager.GetFirstCar() != this)
                {
                    Projectile proj = Instantiate(projectilePrefab) as Projectile;
                    proj.transform.position = launchPt.position;
                    proj.transform.rotation = launchPt.rotation;
                    ...
                    proj.source = this;
                    proj.target = checkpointManager.GetFirstCar();

                    // proj.GetComponent<WaypointProgressTracker>().circuit = projCircuit;
                    proj.GetComponent<WaypointProgressTracker>().CopyFrom(GetComponent<WaypointProgressTracker>());

                    // Clear the prefab so no projectile can be launched
                    projectilePrefab = null;
                }
                else Debug.LogWarning("CANNOT LAUNCH BLUE SHELL - FIRST IN LINE");
            }
    ...
    void Update()
    {
        if (useNitro)
        {
            if (nitroLeft > 0)
            {
                nitroLeft -= nitroDecay * Time.deltaTime;
            }
        }
        else
        {
            nitroLeft = Mathf.Min(nitroLeft + nitroRegenPassive * Time.deltaTime, 1f);
        }

        // Enable/Disable Nitro emission
        foreach (var ps in nitroParticles)
        {
            ps.enableEmission = useNitro && nitroLeft > 0;
        }
    ...
    void FixedUpdate()
    {
        ...
        // Calculate near collision style points
        var carColliders = Physics.OverlapSphere(transform.position, nearCollisionRadius, nearCollisionMask);
        foreach (var c in carColliders)
        {
            if (c.gameObject.GetComponentInParent<CarController>() != this)
                AddStylePoints(stylePointsPerNearCollision);
        }

        // Air time style points
        if (!anyOnGround)
        {
            AddStylePoints(stylePointsPerAirFrame);
        }
    }

    void OnCollisionEnter(Collision col)
    {
        // Damage car if collision is violent enough and hits the right layer
